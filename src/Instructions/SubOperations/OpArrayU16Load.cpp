#include "inc.hpp"
#include "OpArrayU16Load.hpp"

size_t OpArrayU16Load::GetSize()
{
    return 3;
}

std::string_view OpArrayU16Load::GetName()
{
    return "ARRAY_U16_LOAD";
}

void OpArrayU16Load::GetInstructionText(const uint8_t* data, uint64_t addr, size_t& len, std::vector<BinaryNinja::InstructionTextToken>& result)
{
...
bool OpArrayU16Load::GetInstructionLowLevelIL(const uint8_t* data, uint64_t addr, size_t& len, BinaryNinja::LowLevelILFunction& il)
{
    auto r = il.Pop(4);
    auto idx = il.Pop(4);
